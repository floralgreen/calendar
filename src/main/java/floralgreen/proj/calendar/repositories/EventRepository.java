...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value = "SELECT e.id, e.event_title, e.event_start, e.event_end, e.calendar_id, e.record_status_enum FROM events as e JOIN calendars as c ON c.id = e.calendar_id WHERE e.record_status_enum = 'A' AND c.record_status_enum = 'A' AND c.calendar_id = :calendarId AND e.event_start > :fromDate AND e.event_start < :toDate")
    List<Event> findAllCalendarEventsByCalendarIdByDay(@Param(value = "fromDate") OffsetDateTime fromDate,
